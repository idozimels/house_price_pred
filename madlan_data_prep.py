# -*- coding: utf-8 -*-
"""madlan_data_prepimport .ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1KGcg_1GK_p6WbrOYjjSXgGr5m-BqMacZ
"""
...
fname1 = 'output_all_students_Train_v10.xlsx'
data = pd.read_excel(path + fname1)

def prepare_data(data):
    df = data.copy()

    df = df[df['price'].apply(lambda x: bool(re.findall(r'\d+\.?\d*', str(x))))]
    df['price'] = df['price'].apply(lambda x: ''.join(re.findall(r'\d+\.?\d*', str(x))) if bool(re.findall(r'\d+\.?\d*', str(x))) else np.nan).astype(float)
    ...
    df.loc[df['floor_out_of'].str.contains('קומת קרקע מתוך (\d+)'), 'total_floors'] = df['floor_out_of'].str.extract(r'קומת קרקע מתוך (\d+)', expand=False).astype('Int64')
    ...
    df['room_number'] = df['room_number'].apply(lambda x: float(x) if isinstance(x, (int, float)) else pd.to_numeric(x.split()[0], errors='coerce'))
    median_room_numbers = df.groupby(pd.cut(df['Area'], bins=5))['room_number'].median()
    df['room_number'] = df.apply(lambda row: median_room_numbers[row['Area']] if pd.isna(row['room_number']) else row['room_number'],axis=1)
    ...
    df= df.drop(['Street', 'number_in_street', 'city_area', 'num_of_images', 'floor_out_of', 'furniture ', 'publishedDays ','entranceDate ', 'description '], axis=1)
    return df
